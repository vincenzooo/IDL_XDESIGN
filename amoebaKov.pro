...
Function amotry, p, y, psum, func, ihi, fac, range=range
...
; point is better.
  ...
  if n_elements(range) ne 0 then begin
	  out=where(ptry ge range[*,1],count)
	  if count ne 0 then ptry[out]=(range[out,1]+p[out,ihi])/2.
	  out=where(ptry le range[*,0],count)
	  if count ne 0 then ptry[out]=(range[out,0]+p[out,ihi])/2.
  endif
...
Function AmoebaKov, ftol, FUNCTION_NAME=func, FUNCTION_VALUE=y, $
	NCALLS = ncalls, NMAX = nmax, P0 = p0, SCALE=scale, SIMPLEX=p,range=range
...
while ncalls le nmax do begin		;Each iteration
  ...
  ncalls = ncalls + 2
  ytry = amotry(p, y, psum, func, ihi, -1.0,range=range)
  if ytry le y[ilo] then ytry = amotry(p,y,psum, func, ihi, 2.0,range=range) $
  else if ytry ge y[inhi] then begin
    ysave = y[ihi]
    ytry = amotry(p,y,psum,func, ihi, 0.5,range=range)
