...
; CALLING SEQUENCE:
;
; Result = EA_ONAXIS(Energy,Alpha,Acoll,Coatings)
;
; INPUTS:
...
; 
; KEYWORD PARAMETERS:
; OUTFOLDER: If provided, resulting effective area is saved here as text matrix
;   with energy in first column and 1-line header.
;
; OUTPUTS:
...
function EA_onaxis,energy,alpha,acoll,coatings,roughness,$
  outfolder=outfolder,extract=extract
...
EA_m=EA_onaxis(energy,alpha,acoll,coatings,roughness,$
  outfolder=outfolder)
...
maketif,outfolder+path_sep()+'EA_tot'
 ...
 ener=(findgen(10))+0.2
 EA_m=EA_onaxis(ener,[0.6584,0.6013,0.544],[0.6843,0.5609,0.4514],'Ir',4.) ;small telescope
 EA_tot=total(EA_m,1)
 window,/free
 plot,ener,EA_tot,xtitle='Energy (keV)',ytitle='Aeff (cm^2)',$
   title='Total telescope On-axis effective area',yrange=[0,max(EA_tot)*1.1]
