...
; 
; IRT_REFLEX calculates reflectivity for a list of photons, each one with angle and energy
...
function tref2d,LAM,an,material
  ;reflex 2D
  ;+
  ;Given vectors (or scalars) LAM_M and AN,
  ;  return a 2D array [nenergy,nangle] with reflectivity as a function of energy (in keV) and angle (in radians).
  ;  energy and angle are passed to TREF as vectors of same length (as a list of photons), whose interface is kept unchanged
  ;  for compatibility with IRT.
  ;  
  ;  Note that this is the most general approach, but not the fastest, as the reflectivity is recalculated nph times even when energies are repeated. It is not a big effect with monolayers and reflectivity only.
  ;-

  if (size(LAM_V))[0] eq 0 then LAM=[LAM] else LAM = LAM_V
  if (size(an))[0] eq 0 then angle=[an] else angle = an
  ...
  nph=n_elements(LAM)*n_elements(angle)
  
  a=reform(transpose(Rebin(angle, n_elements(angle), n_elements(LAM))),nph)
  LAM_M=reform(Rebin(LAM, n_elements(LAM), n_elements(angle)),nph)
  rind=load_nk(LAM_M,material) 
  
  R=IRT_REFLEX(a,Rind)
  
  return, reform(r,n_elements(LAM), n_elements(angle))
