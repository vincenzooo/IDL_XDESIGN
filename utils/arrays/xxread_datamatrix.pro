...
;read file, count lines. Determine number of columns from the last line
...
while ~ EOF(unit0) do begin
  ...
  if i lt s then s=s-1 else begin  
    ...
    ;ignore comments
    if n_elements(comment) eq 0 || in(strmid(strtrim(lines,2),0,1),comment) ne 1 then begin
...
if n_elements(delimiter) eq 0 then ncols=n_elements(strsplit(lastline)) $
    else ncols=n_elements(strsplit(lastline,delimiter))  ;can be updated using regex
